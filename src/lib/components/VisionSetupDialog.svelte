  ...
  <Dialog.Content class="max-w-md">
    ...
    <div class="grid gap-4 py-2">
      ...
      <Field.Field>
        ...
        <Field.Description>
          示例：近视 300 度填写 300；暂不考虑远视填 0
        </Field.Description>
      </Field.Field>

      <!-- 散光度数 -->
      <Field.Field>
        <Field.Label for="astigmatism-input">
          散光度数（度）
        </Field.Label>
        <div class="relative">
          <input
      ...
      <!-- 计算按钮 -->
      <Button onclick={handleCompute} class="w-full">
        计算匹配方案
      </Button>
      ...
      {#if recommendation}
        ...
        <div class="rounded-xl border bg-muted/40 p-4 space-y-3">
          ...
          <div class="rounded-lg border bg-background px-3 py-2 space-y-1">
            ...
            <div class="flex justify-between text-sm">
              ...
              <span class="font-semibold">
                {rec.durationWeeks * 7 * rec.dailyMinutes} 分钟
              </span>
            </div>
          </div>

          <p class="text-xs text-muted-foreground leading-relaxed">
            {rec.cycleDescription}
          </p>
          ...
          {:else}
            <Button onclick={handleApply} variant="default" class="w-full">
              应用到训练器
            </Button>
          {/if}
        </div>
