    ...
    <div {@attach attachHudContentSizer} class="flex w-max items-center gap-2">
      ...
      <div class="flex shrink-0 items-center gap-2 md:hidden">
        ...
        >
          <Select.Trigger
            ...
            aria-label={`训练模式：${getPresetName(settings.presetId)}`}
            title={`训练模式：${getPresetName(settings.presetId)}`}
          >
          ...
          <div class="flex shrink-0" in:hudControlTransition>
            ...
            >
              <Select.Trigger
                ...
                class="pressable-ui size-9 justify-center rounded-full p-0 [&>svg:last-child]:hidden"
                aria-label={`运动路径：${getPatternName(settings.patternId)}`}
                title={`运动路径：${getPatternName(settings.patternId)}`}
              >
        ...
        {:else if isLilacChaserMode}
          <div class="flex shrink-0" in:hudControlTransition>
            ...
            >
              <Select.Trigger
                class="pressable-ui size-9 justify-center rounded-full p-0 [&>svg:last-child]:hidden"
                aria-label={`幻影追踪球颜色：${getLilacChaserColorName(settings.lilacChaserBallColor)}`}
                title={`幻影追踪球颜色：${getLilacChaserColorName(settings.lilacChaserBallColor)}`}
              >
                <svg
                  viewBox="0 0 12 12"
      ...
      <nav class="flex shrink-0 items-center gap-2" aria-label="操作按钮">
        <Button
          ...
          variant="outline"
          size="icon"
          aria-label={motionPaused ? "继续运动" : "暂停运动"}
